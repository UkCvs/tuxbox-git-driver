...
#include <avia/avia_av.h>
#include <avia/avia_gt_config.h>
#include <avia/avia_gt_dmx.h>
#include <avia/avia_gt_pcm.h>
#include <avia/ts_pes_header.h>

#include "dbox2_avia_av_napi.h"
#include "dbox2_napi_core.h"
...
int __init avia_av_napi_init(void)
{
	...
	if ((result = dvb_register_device(dbox2_napi_get_adapter(), &video_dev, &avia_av_napi_video_dev, NULL, DVB_DEVICE_VIDEO)) < 0) {
		printk(KERN_ERR "%s: dvb_register_device (video) failed (errno = %d)\n", __FILE__, result);
		goto fail_1;
	}

	if ((result = dvb_register_device(dbox2_napi_get_adapter(), &audio_dev, &avia_av_napi_audio_dev, NULL, DVB_DEVICE_AUDIO)) < 0) {
...
MODULE_DESCRIPTION("dbox2 DVB decoder driver");
